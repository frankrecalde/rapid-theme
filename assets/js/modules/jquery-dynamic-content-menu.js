...
(function (factory) {
  ...
  // Constants
  var VERSION = '1.0.0';
  var PLUGIN_NS = 'DynamicContentMenu';
  var PLUGIN_NAME = 'dynamicContentMenu';
  var pluginClassName = PLUGIN_NAME;
  // pluginClass = "." + pluginClassName,
  var pluginFocusClassName = 'active';
  var pluginHideClassName = PLUGIN_NAME + '_hide';
  // pluginHideClass = "." + pluginHideClassName,
  var headerClassName = PLUGIN_NAME + '__header';
  var headerClass = '.' + headerClassName;
  var subheaderClassName = PLUGIN_NAME + '__subheader';
  var subheaderClass = '.' + subheaderClassName;
  var itemClassName = PLUGIN_NAME + '__item';
  var itemClass = '.' + itemClassName;
  var extendPageClassName = PLUGIN_NAME + '__extend-page';
  ...
  DynamicContentMenu.prototype._generateHtmlMenu = function () {
    ...
    var ul;
    var { ignoreSelector } = self.options;

    // If the selectors option has a comma within the string
    if (this.options.selectors.indexOf(',') !== -1) {
      // Grabs the first selector from the string
      firstElem = $(this.options.context).find(
        this.options.selectors.replace(/ /g, '').substr(0, this.options.selectors.indexOf(',')),
      );
    }
          ...
          $('html, body')
            ...
            .done(function () {
              ...
              if (self.options.extendPage) {
                ...
                if (
                  (self.webkit && winScrollTop >= scrollHeight - winHeight - self.options.extendPageOffset) ||
                  (!self.webkit && winHeight + winScrollTop > docHeight - self.options.extendPageOffset)
                ) {
    ...
    return this.each(function () {
      ...
      if (typeof option === 'string') {
        if (!$.isFunction(instance[option]) || option.charAt(0) === '_') {
          $.error("no such method '" + option + "' for " + PLUGIN_NAME + ' plugin instance');
        }
